...
int load_csv_as_pcb (char* textfile, linkedlist_t** pcb_llist) {
	const char* delim = ",";
	char text[300];
	...
	while (fgets(text, 300, file)) {
		...
		pcb->priority = -1; // default for non-priority scheduling
		char* temp = strtok(NULL, delim);
		if (temp != NULL) {
			pcb->priority = atoi(temp);
		}
		pcb->state = NEW_STATE;
		pcb->exec_time = 0;
		pcb->wait_time = 0;
		...
		append_node(pcb_llist, node, pcb->priority);
	}
...
void string_pcb(char* o_file, pcb_t* pcb, int sys_time, char* old_state) {
