...
int main(int argc, char** argv) {
	...
	// if invalid scheduling is input, it will default to FCFS
	...
	int load_result = load_csv_as_pcb(i_file, &pcb_llist, sched);
	...
	while (system_run) {
		...
		if ( 	exec_llist->head != NULL && 
			...
			string_pcb(o_file, new_term_node->data, system_time, EXEC_STATE, sched);
			process_counter +=1;
		...
		if (sched == 2 && exec_llist->head != NULL && curr_exec_time >= TIME_QUANTUM){
			...
			new_ready_node->data->enter_ready = system_time;
			string_pcb(o_file, new_ready_node->data, system_time, EXEC_STATE, sched);
		}
		...
		while ( pcb_llist->head != NULL && 
			...
			new_ready_node->data->state = READY_STATE;
			new_ready_node->data->enter_ready = system_time;
			string_pcb(o_file, new_ready_node->data, system_time, NEW_STATE, sched);
		}
		...
		while ( wait_llist->head != NULL && 
			...
			new_ready_node->data->state = READY_STATE;
			new_ready_node->data->enter_ready = system_time;
			string_pcb(o_file, new_ready_node->data, system_time, WAITING_STATE, sched);
		...
		if ( 	exec_llist->head != NULL && 
			...
			string_pcb(o_file, new_wait_node->data, system_time, EXEC_STATE, sched);
		}
		...
		if ( 	ready_llist->head != NULL &&
			...
			new_exec_node->data->exit_ready = system_time;
			new_exec_node->data->total_wait += new_exec_node->data->exit_ready - new_exec_node->data->enter_ready;
			curr_exec_time = 0;
			string_pcb(o_file, new_exec_node->data, system_time, READY_STATE, sched);
		}
	...
	while ( curr_aw_node != NULL) {
		total_wait += curr_aw_node->data->total_wait;
